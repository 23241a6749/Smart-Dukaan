    ...
    return (
        <div className="p-4 safe-area-bottom space-y-5 pb-32">
            <div className="flex flex-wrap items-center justify-between gap-3">
                <div>
                    <h2 className="text-2xl font-black text-gray-900">{t['Expiry & Waste Command Center'] || 'Expiry & Waste Command Center'}</h2>
                </div>
                <button
                    onClick={handleRefresh}
                    className="inline-flex items-center gap-2 px-4 py-2 rounded-xl bg-primary-green text-white font-bold"
                >
                    <RefreshCw size={16} className={refreshing ? 'animate-spin' : ''} /> {t['Refresh']}
                </button>
            </div>

            <div className="grid grid-cols-2 md:grid-cols-5 gap-3">
                <MetricCard icon={<AlertTriangle size={16} />} label={t['Urgent (0-3d)']} value={summary.urgent_3d} color="bg-red-50 text-red-700" />
                <MetricCard icon={<Clock3 size={16} />} label={t['This Week']} value={summary.week_7d} color="bg-orange-50 text-orange-700" />
                <MetricCard icon={<CalendarClock size={16} />} label={t['This Month']} value={summary.month_30d} color="bg-yellow-50 text-yellow-700" />
                <MetricCard icon={<PackageX size={16} />} label={t['Expired']} value={summary.expired} color="bg-gray-100 text-gray-700" />
                <MetricCard icon={<Recycle size={16} />} label={t['At Risk Value']} value={`₹${summary.totalValueAtRisk.toFixed(0)}`} color="bg-emerald-50 text-emerald-700" />
            </div>

            <div className="grid grid-cols-1 xl:grid-cols-3 gap-4">
                <div className="xl:col-span-2 bg-white rounded-2xl shadow-sm border border-gray-100 p-4">
                    <div className="flex items-center justify-between mb-3">
                        <h3 className="font-black text-lg">{t['Priority Risk Queue']}</h3>
                        <span className="text-xs text-gray-500">{t['Top']} {topRiskItems.length} {t['items']}</span>
                    </div>
                    <div className="space-y-3">
                        {topRiskItems.length === 0 && (
                            <div className="p-4 rounded-xl bg-emerald-50 text-emerald-700 text-sm font-semibold">{t['No records found']}</div>
                        )}
                                ...
                                <div className="flex flex-wrap items-center justify-between gap-2">
                                    <div>
                                        ...
                                        <p className="text-xs text-gray-500">
                                            {t['Qty']} {item.batchId?.quantityAvailable} • {item.daysToExpiry < 0 ? t['Expired'] : `${item.daysToExpiry} ${t['days left']}`} • {t['Risk']} ₹{item.valueAtRisk.toFixed(0)}
                                        </p>
                                    </div>
                                    <span className="text-xs font-bold px-2 py-1 rounded-full bg-white border border-gray-200">{item.suggestedAction}</span>
                                </div>
                                <div className="mt-3 flex flex-wrap gap-2">
                                    <button onClick={() => markAction(item._id, 'in_progress')} className="px-3 py-1.5 rounded-lg bg-blue-50 text-blue-700 text-xs font-bold">{t['In Progress']}</button>
                                    <button onClick={() => markAction(item._id, 'done', { mode: 'discount' })} className="px-3 py-1.5 rounded-lg bg-amber-50 text-amber-700 text-xs font-bold">{t['Discount Done']}</button>
                                    <button onClick={() => markAction(item._id, 'done', { mode: 'bundle' })} className="px-3 py-1.5 rounded-lg bg-purple-50 text-purple-700 text-xs font-bold">{t['Bundle Done']}</button>
                                    <button onClick={() => markAction(item._id, 'ignored')} className="px-3 py-1.5 rounded-lg bg-gray-200 text-gray-700 text-xs font-bold">{t['Ignore']}</button>
                                </div>
                            </div>
                        ))}
                    </div>
                ...
                <div className="space-y-4">
                    <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-4">
                        <h3 className="font-black text-base mb-3">{t['Add Stock Batch']}</h3>
                        <div className="space-y-2">
                            ...
                            <div className="grid grid-cols-2 gap-2">
                                <input value={batchForm.quantity} onChange={(e) => setBatchForm((prev) => ({ ...prev, quantity: e.target.value }))} className="rounded-xl border border-gray-200 p-2.5 text-sm" type="number" min="1" placeholder={t['Qty']} />
                                <input value={batchForm.costPricePerUnit} onChange={(e) => setBatchForm((prev) => ({ ...prev, costPricePerUnit: e.target.value }))} className="rounded-xl border border-gray-200 p-2.5 text-sm" type="number" min="0" placeholder={t['Cost/unit']} />
                            </div>
                            <div className="grid grid-cols-2 gap-2">
                                ...
                                <input value={batchForm.batchCode} onChange={(e) => setBatchForm((prev) => ({ ...prev, batchCode: e.target.value }))} className="rounded-xl border border-gray-200 p-2.5 text-sm" placeholder={t['Batch code']} />
                            </div>
                            <button onClick={handleCreateBatch} className="w-full rounded-xl bg-primary-green text-white py-2.5 text-sm font-bold inline-flex items-center justify-center gap-2"><Save size={15} /> {t['Save Batch']}</button>
                        </div>
                    </div>

                    <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-4">
                        <h3 className="font-black text-base mb-3">{t['Log Waste']}</h3>
                        <div className="space-y-2">
                            <select
                                ...
                                className="w-full rounded-xl border border-gray-200 p-2.5 text-sm"
                            >
                                <option value="">{t['Select risky batch']}</option>
                            ...
                            <div className="grid grid-cols-2 gap-2">
                                <input value={wasteForm.quantity} onChange={(e) => setWasteForm((prev) => ({ ...prev, quantity: e.target.value }))} className="rounded-xl border border-gray-200 p-2.5 text-sm" type="number" min="0" placeholder={t['Qty']} />
                            ...
                            <button onClick={handleLogWaste} className="w-full rounded-xl bg-red-600 text-white py-2.5 text-sm font-bold inline-flex items-center justify-center gap-2"><Trash2 size={15} /> {t['Log Waste']}</button>
                        </div>
                    </div>
                </div>
            </div>
            ...
            <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-4">
                <h3 className="font-black text-lg mb-3">{t['Waste & Recovery Snapshot']}</h3>
                <div className="grid grid-cols-1 md:grid-cols-3 gap-3 mb-3">
                    <MetricCard icon={<Trash2 size={16} />} label={t['Waste Value (30d)']} value={`₹${wasteKPI.totalWasteValue.toFixed(0)}`} color="bg-red-50 text-red-700" />
                    <MetricCard icon={<PackageX size={16} />} label={t['Waste Qty (30d)']} value={wasteKPI.totalWasteQty.toFixed(2)} color="bg-orange-50 text-orange-700" />
                    <MetricCard icon={<CheckCircle2 size={16} />} label={t['Recovered Actions']} value={wasteKPI.recoveredActions} color="bg-emerald-50 text-emerald-700" />
                </div>

                <div className="space-y-2">
                    {translatedWasteHistory.length === 0 && <div className="text-sm text-gray-500">{t['No records found']}</div>}
                    {translatedWasteHistory.map((entry) => (
                        <div key={entry._id} className="text-sm p-2.5 rounded-lg bg-gray-50 border border-gray-100 flex justify-between gap-3">
                            ...
                            <span className="text-gray-600">{t['Qty']} {entry.quantity} • {t['Loss']} ₹{entry.estimatedLoss.toFixed(0)}</span>
                        </div>
                    ))}
                </div>
            </div>
